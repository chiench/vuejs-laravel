    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          Or
          {{ ' ' }}
          <router-link :to="{name:'Login'}" class="font-medium text-indigo-600 hover:text-indigo-500"> You already have account Sign in Here </router-link>
        </p>
      </div>
      <form class="mt-8 space-y-6" @click="register">
        ...
        <div class="rounded-md shadow-sm -space-y-px">
          ...
          <div>
            ...
            <input v-model="confirm_password" id="confirm-password" name="confirm-password" type="password" autocomplete="current-password" required="" class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Confirm Password" />
          </div>
        </div>
